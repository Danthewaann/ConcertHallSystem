...
public class MainGUI extends JFrame 
{
    ...
    private Seat[] selectedConcertSeats;    
    private int nConcerts_;
    private int selectedConcertIndex_;
    private final File CONCERTS_FILE = new File("CONCERT_LIST.txt");
    private Cursor selectCursor = new Cursor(Cursor.HAND_CURSOR);
    ...
    public MainGUI() 
    {
        ...
        catch(IOException ex)
        {
            JOptionPane.showMessageDialog(
                null, "A fatal error occurred...\n" + ex.getMessage(), 
                "A Fatal Error Occurred", JOptionPane.PLAIN_MESSAGE
            );
            System.exit(0);
        }
    }
    ...
    private void initComponents() {
        ...
        createNewConcertButton.setCursor(selectCursor);
        ...
        selectConcertButton.setCursor(selectCursor);
        ...
        exitProgramButton.setCursor(selectCursor);
        ...
        obtainReportButton.setCursor(selectCursor);
        ...
        setSectionPriceButton.setCursor(selectCursor);
        ...
        queryBySeatButton.setCursor(selectCursor);
        ...
        queryByCustomerButton.setCursor(selectCursor);
        ...
        mainMenuButtonFromSeats.setCursor(selectCursor);
        ...
        saveConcertButton.setCursor(selectCursor);
        ...
        mainMenuButtonFromCreate.setCursor(selectCursor);
        ...
        dayDropDownList.setCursor(selectCursor);
        ...
        monthDropDownList.setCursor(selectCursor);
        ...
        yearDropDownList.setCursor(selectCursor);
        ...
        createConcertButton.setCursor(selectCursor);
        ...
        mainMenuButtonFromSelect.setCursor(selectCursor);
        ...
        viewConcertButton.setCursor(selectCursor);
        ...
        concertDropDownList.setCursor(selectCursor);
    ...
    {       
        ...
        catch(FileNotFoundException i) //If the concerts file doesn't exist, create a new one
        {                 
            PrintWriter concertsFile = new PrintWriter(CONCERTS_FILE);
            concertsFile.print("");                                               
        }            
    ...
    //Loads all concerts and their seats in the application from file
    private void loadConcertsFromFile() throws FileNotFoundException
    {
        Scanner concertInput;
        int index;       
        {
            concerts = new Concert[100]; //Create 100 references to null concert objects
            concertInput = new Scanner(CONCERTS_FILE);
            while(concertInput.hasNextLine())
            {
                Concert newConcert = new Concert(concertInput);
                this.nConcerts_++;
                index = this.nConcerts_ - 1;
                this.concerts[index] = newConcert;
                this.concertDropDownList.addItem(
                    newConcert.getName() + " | " + newConcert.getDateWithSlashes()
                );
    ...
    //Saves all concerts and their seats information to file
    private void saveConcertsToFile() throws FileNotFoundException
    {
        PrintWriter concertOutput;
        PrintWriter seatOutput;              
        
        //Open up output to concerts file, printing out every concert loaded on the system to the file
        concertOutput = new PrintWriter(CONCERTS_FILE);
        for(int i = 0; i < this.nConcerts_; i++)
        {
            concertOutput.print(
                this.concerts[i].getName().toUpperCase() + " " + this.concerts[i].getDate() + " "
            );
            concertOutput.println(
                this.concerts[i].getSectionPrice("GOLD") 
                + " " + this.concerts[i].getSectionPrice("SILVER") 
                + " " + this.concerts[i].getSectionPrice("BRONZE")
            );

            //Create the seats file for the current concert, and print out all the booked
            //seats for that concert to the file
            File concertSeats = new File(
                "CONCERTS_INFO/" + this.concerts[i].getName().toUpperCase() 
                + "_" + this.concerts[i].getDate() + "_SEATS.txt"
            );
            seatOutput = new PrintWriter(concertSeats);
            
            this.selectedConcertSeats = this.concerts[i].getSeats();
            for(int j = 0; j < Constant.TOTAL_SEATS; j++)
            {                
                if(this.selectedConcertSeats[j].getStatus() == true)
                {
                    seatOutput.print(this.selectedConcertSeats[j].getRow() + " ");
                    seatOutput.print(this.selectedConcertSeats[j].getNumber() + " ");
                    seatOutput.print(this.selectedConcertSeats[j].getBookee() + " ");
                    seatOutput.println(this.selectedConcertSeats[j].getEntitled());
                }
            }
            seatOutput.close();              
        }
    ...
    private void exitProgramButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exitProgramButtonActionPerformed
        ...
        int option = JOptionPane.showOptionDialog(
            this, "Are you sure you want to exit?", "Exit Option", JOptionPane.YES_NO_OPTION, 
        ...
        if(option == 0)
        {
            try
            {
                ...
                JOptionPane.showMessageDialog(
                    ...
                    + "Application will now exit...", "Save & Exit", JOptionPane.PLAIN_MESSAGE
                );
                System.exit(0);
            }
            catch(FileNotFoundException ex)
            {
                JOptionPane.showMessageDialog(
                    ...
                    "Error: Save Operation Failed", JOptionPane.PLAIN_MESSAGE
                );               
                System.exit(0);                       
            }
        } 
    ...
    private void setSectionPriceButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_setSectionPriceButtonActionPerformed
        String seatSectionInput = JOptionPane.showInputDialog(
            this, "Please input seat section you want to change:\n" + "e.g Gold, Silver or Bronze", 
            "Change Seat Section price", JOptionPane.PLAIN_MESSAGE               
        );
        String seatSection = seatSectionInput.trim();
        if(seatSection.length() > 0)
        {
            int i = 0;            
            boolean found = false;
            while(i < Constant.SEAT_SECTIONS.length && !found)
            {
                if(seatSection.compareToIgnoreCase(Constant.SEAT_SECTIONS[i]) == 0)
                {
                    found = true;
                    String newPrice = (JOptionPane.showInputDialog(
                        this, "Please input new price: (e.g 60.00 or 60) ", 
                        "Change Seat Section Price", JOptionPane.PLAIN_MESSAGE)
                    );
                    ...
                    {                       
                        try
                        {
                            ...
                            this.concerts[this.selectedConcertIndex_].setSectionPrice(seatSection, finalPrice);
                            
                            JOptionPane.showMessageDialog(
                                this, seatSection.toUpperCase() + " section price changed to: £" 
                                + concerts[this.selectedConcertIndex_].getSectionPrice(seatSection), 
                                "Change Seat Section price", JOptionPane.PLAIN_MESSAGE
                            );     
                        }
                        ...
                        {
                            JOptionPane.showMessageDialog(
                                this, "Inputted value is not a number", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                            );
                        }                                                       
                    }
                    else
                    {
                        JOptionPane.showMessageDialog(
                            this, "Please input a new price", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                        );
                    }
                }   
                else
            ...
            {                     
                JOptionPane.showMessageDialog(
                    this, "Inputted seat section does not exist", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                );          
            }
        }
        else
        {
            JOptionPane.showMessageDialog(
                this, "Please input a seat section", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
            );
        }
    ...
    private void queryBySeatButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_queryBySeatButtonActionPerformed
        String seatRowInput = JOptionPane.showInputDialog(
            this, "Please input row you want to query:\n" + "(From A to I)", "Query By Seat", JOptionPane.PLAIN_MESSAGE
        );
        
        String seatRow = seatRowInput.trim();
        if(seatRow.length() > 0)
        {           
            int i = 0;
            boolean foundRow = false;
            while(i < Constant.SEAT_ROWS.length && !foundRow)
            {
                if(seatRow.compareToIgnoreCase(Constant.SEAT_ROWS[i]) == 0)
                {
                    foundRow = true;
                    String seatNumInput = JOptionPane.showInputDialog(
                        this, "Please input seat number you want to query:\n" 
                        + "(From 1 to 10)", "Query By Seat", JOptionPane.PLAIN_MESSAGE
                    );
                    String seatNum = seatNumInput.trim();
                    if(seatNum.length() > 0)
                    {                      
                        ...
                        try
                        {
                            ...
                            {
                                JOptionPane.showMessageDialog(
                                    this, concerts[this.selectedConcertIndex_].queryBySeat(seatRow, seatNumToInt), 
                            ...
                            {
                                JOptionPane.showMessageDialog(
                                    this,"Inputted seat number does not exist", 
                                    "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                                );
                            }
                        }
                        catch(NumberFormatException n)
                        ...
                        {
                            JOptionPane.showMessageDialog(
                                this, "Please input a seat number",
                                "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                            );
                        }                                                                                                  
                    }
                    ...
                    {
                        JOptionPane.showMessageDialog(
                            this, "Please input a seat number", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                        );
                    }                                      
                }
                else
            ...
            {
                JOptionPane.showMessageDialog(
                    this, "Inputted row does not exist", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
                );
            }
        }
        else
        {
            JOptionPane.showMessageDialog(
                this, "Please input a row", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
            );
        }
    ...
    private void queryByCustomerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_queryByCustomerButtonActionPerformed
        String customerNameInput = JOptionPane.showInputDialog(
            this, "Please input customer fullname that you want to query: ", 
            "Query By Customer", JOptionPane.PLAIN_MESSAGE
        );
        String customerName = customerNameInput.trim();
        if(customerName.length() > 0)
        {
            JOptionPane.showMessageDialog(
                this, concerts[this.selectedConcertIndex_].queryByCustomer(customerName), 
                "Query Result", JOptionPane.PLAIN_MESSAGE
            );
        }
        else
        {
            JOptionPane.showMessageDialog(
                this, "Please input a customer name", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
            );
        }
    ...
    private void createConcertButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_createConcertButtonActionPerformed
        String newConcertNameInput = this.ConcertNameInputField.getText();
        String newConcertName = newConcertNameInput.trim();
        if(newConcertName.length() > 0)
        {
            ...
            while(i < this.nConcerts_ && !concertAlreadyExists) 
            {
                if(this.concerts[i].getName().compareToIgnoreCase(newConcertName) == 0)
                {                    
                    if(this.concerts[i].getDateWithSlashes().compareTo(dateWithSlashes) == 0)
                    {
                        concertAlreadyExists = true;
                        Object[] options = {"Yes", "Cancel"};
                        int option = JOptionPane.showOptionDialog(
                            this, "This concert already exists, do you want to override it?", 
                            "Concert Already Exists", JOptionPane.YES_NO_OPTION, 
                            JOptionPane.QUESTION_MESSAGE, null, options, options[0]
                         );
                        
                        if(option == 0)
                        {
                            this.concerts[i] = new Concert(newConcertName, day, month, year);
                            this.concertDropDownList.removeItemAt(i);
                            this.concertDropDownList.addItem(this.concerts[i].getName().toUpperCase()
                                + " | " + this.concerts[i].getDateWithSlashes());
                            
                            this.selectedConcertIndex_ = i;
                            
                            JOptionPane.showMessageDialog(
                                this, "Created a new concert called:\n" + this.concerts[selectedConcertIndex_].getName() 
                                + "\n" + "For date: " + this.concerts[selectedConcertIndex_].getDateWithSlashes(), 
                                "New Concert", JOptionPane.PLAIN_MESSAGE
                            );
            ...
            {               
                this.nConcerts_++;
                this.selectedConcertIndex_ = this.nConcerts_ - 1;
                this.concerts[selectedConcertIndex_] = new Concert(newConcertName, day, month, year);
                this.concertDropDownList.addItem(
                    this.concerts[this.selectedConcertIndex_].getName().toUpperCase() 
                    + " | " + this.concerts[this.selectedConcertIndex_].getDateWithSlashes()
                );
        ...
        {                                      
            JOptionPane.showMessageDialog(
                this, "No name supplied for new concert", "Error: Invalid Input", JOptionPane.PLAIN_MESSAGE
            );
        }                            
    ...
    private void displaySeatingPlan()
    {
        ...
        //create a seatButtonHandler array to store all the event handler for use of
        //the seat icons in the seatIcons array
        this.selectedConcertSeats = this.concerts[this.selectedConcertIndex_].getSeats();
        SeatHandler[] seatHandlers = new SeatHandler[seatIcons.length]; 
        
        //Attach a SeatHandler to every seatIcon
        for(int i = 0; i < seatIcons.length; i++)
        {            
            //If a seatIcon has an action listener attached to it, remove it          
            if(seatIcons[i].getActionListeners().length > 0)
            {                
                seatIcons[i].removeActionListener(seatHandlers[i]);                                              
            }
            //If no action listener is attached to it, attach a SeatHandler to it 
            else
            {
                seatIcons[i].setCursor(selectCursor);
                String seatRow = this.selectedConcertSeats[i].getRow();
                int seatNum = this.selectedConcertSeats[i].getNumber();
                seatHandlers[i] = new SeatHandler(
                    this, this.concerts, this.selectedConcertIndex_, 
                    seatIcons[i], seatRow, seatNum
                );
                seatIcons[i].addActionListener(seatHandlers[i]);
            }           
        }        
        
        //Set all seats to their respective colours for the selected concert
        for(int i = 0; i < seatIcons.length; i ++)
        {
            if(i < 30)
            {
                if(this.selectedConcertSeats[i].getStatus())
                {
        ...
        this.setTitle(
            "CONCERT | " + this.concerts[selectedConcertIndex_].getName().toUpperCase() 
            + " " + this.concerts[selectedConcertIndex_].getDateWithSlashes()
        );     
    ...
    private void viewConcertButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewConcertButtonActionPerformed
        //Gets the selected concerts newConcertName and then displays its seating plan
        String concertName = this.concertDropDownList.getSelectedItem().toString();
        int divider = concertName.indexOf("|");
        String finalConcertName = concertName.substring(0, divider-1);
        
        int i = 0;
        boolean foundConcert = false;
        while(i < this.nConcerts_ && !foundConcert)
        {
            if(finalConcertName.compareToIgnoreCase(this.concerts[i].getName()) == 0)
            {
                foundConcert = true;
                this.selectedConcertIndex_ = i;
                this.selectConcertPanel.setVisible(false);
    ...
    private void obtainReportButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_obtainReportButtonActionPerformed
        String concertReport = this.concerts[this.selectedConcertIndex_].getReport();        
        
        JOptionPane.showMessageDialog(
            this, concertReport, "Concert Report", JOptionPane.PLAIN_MESSAGE
        );
    ...
    private void saveConcertButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveConcertButtonActionPerformed
        try
        {
            this.saveConcertsToFile();
            JOptionPane.showMessageDialog(
                this, "Successfully Saved Concert:\n" + this.concerts[this.selectedConcertIndex_].getName() 
                + " " + this.concerts[this.selectedConcertIndex_].getDateWithSlashes() + " to file!", 
